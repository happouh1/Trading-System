"""Append-only Phase 6E audit-review assembly and persistence."""
...
def _time(value: datetime) -> str:
    if value.tzinfo is None or value.utcoffset() is None:
        raise ValueError("audit review timestamps must be timezone-aware")
...
class ObservationAuditReviewRegistry:
    def __init__(
        self, repository: SQLiteRepository, config: ObservationAuditReviewConfig
    ) -> None:
    ...
    def create(
        ...
        *,
        ...
        if not all((export_id, verification_id, reviewer_id, source_revision)):
            raise ValueError("audit review source and reviewer identities are required")
        export_row = self.repository.connection.execute(
            """SELECT exported_at, artifact_hash, code_version, payload_json, payload_hash
               FROM operations_observation_audit_exports WHERE export_id = ?""",
            (export_id,),
        ).fetchone()
        if export_row is None:
            raise ValueError("unknown observation audit export")
        manifest = _root(str(export_row[3]), str(export_row[4]), "audit export manifest")
        if (
            manifest.get("export_id") != export_id
            or manifest.get("artifact_hash") != str(export_row[1])
            or manifest.get("code_version") != str(export_row[2])
            or str(export_row[2]) != PACKAGE_VERSION
        ):
            raise ValueError("audit export manifest identity or code version mismatch")
        ...
        if verification_row is None:
            raise ValueError("unknown audit export verification")
        verification = _root(
            str(verification_row[4]), str(verification_row[5]), "audit export verification"
        )
        if str(verification_row[0]) != export_id or verification.get("export_id") != export_id:
            raise ValueError("audit export verification link mismatch")
        if (
            str(verification_row[2]) != "VERIFIED"
            or verification.get("status") != "VERIFIED"
            or verification.get("reasons") != []
        ):
            raise ValueError("audit review requires a VERIFIED export")
        if (
            verification.get("verification_id") != verification_id
            or verification.get("expected_hash") != str(export_row[1])
            or verification.get("actual_hash") != str(export_row[1])
            ...
            raise ValueError("audit export verification evidence mismatch")
        verified_at = datetime.fromisoformat(str(verification_row[1]).replace("Z", "+00:00"))
        if reviewed_at < verified_at:
            raise ValueError("audit review cannot predate export verification")
        if supersedes_review_id is not None:
            prior = self.repository.connection.execute(
                """SELECT export_id, reviewer_id, reviewed_at
                   FROM operations_observation_audit_reviews WHERE review_id = ?""",
                (supersedes_review_id,),
            ).fetchone()
            if prior is None:
                raise ValueError("unknown superseded audit review")
        ...
        return ObservationAuditReview.create(
            export_id=export_id,
            ...
            export_manifest_hash=str(export_row[4]),
            verification_payload_hash=str(verification_row[5]),
            source_revision=source_revision,
            config=self.config,
        )

    def insert(self, review: ObservationAuditReview) -> bool:
        if review.config_hash != self.config.config_hash:
            raise ValueError("audit review configuration hash mismatch")
        payload_json = canonical_json(review)
        payload_hash = canonical_hash(review)
        values = (
            review.review_id,
            review.export_id,
        ...
        cursor = self.repository.connection.execute(
            """INSERT OR IGNORE INTO operations_observation_audit_reviews
        ...
        if cursor.rowcount == 0:
            ...
            if stored != values:
                raise ValueError(f"conflicting observation audit review: {review.review_id}")
            return False
    ...
    def status(self, export_id: str) -> tuple[tuple[dict[str, Any], ...], dict[str, int]]:
        exists = self.repository.connection.execute(
            "SELECT 1 FROM operations_observation_audit_exports WHERE export_id = ?",
            (export_id,),
        ).fetchone()
        if exists is None:
            raise ValueError("unknown observation audit export")
        ...
        superseded = {
            ...
            for row in self.repository.connection.execute(
                """SELECT supersedes_review_id FROM operations_observation_audit_reviews
                   WHERE export_id = ? AND supersedes_review_id IS NOT NULL""",
                (export_id,),
            ).fetchall()
        }
        reviews: list[dict[str, Any]] = []
        counts = {verdict.value: 0 for verdict in AuditReviewVerdict}
        counts["TOTAL"] = len(rows)
        counts["ACTIVE"] = 0
        counts["SUMMARY_ELIGIBLE"] = 0
        for row in rows:
            value = _root(str(row[3]), str(row[4]), "audit review")
