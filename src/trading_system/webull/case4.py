"""Offline Case-4 short-cover netting validation harness."""
...
class Case4Transport(Protocol):
    ...
    def preview_exact_cover(
        self, account_id: str, order: WebullExitOrder
    ) -> WebullResponse: ...

    def place_exact_cover(
        self, account_id: str, order: WebullExitOrder
    ) -> WebullResponse: ...
...
def case4_client_order_id(session_id: str) -> str:
    return client_order_id(f"{session_id}:phase3d5:case4:cover:AAPL")


def exact_case4_order(session_id: str) -> WebullExitOrder:
    return WebullExitOrder(
        case4_client_order_id(session_id),
        "AAPL",
        WebullSide.BUY,
        1,
        "MARKET",
...
def validate_case4_order(session_id: str, order: WebullExitOrder) -> None:
    if order != exact_case4_order(session_id):
        raise ValueError("order does not match the exact approved Case-4 fixture")
...
def _detail_item(response: WebullResponse, client_id: str) -> Mapping[str, object]:
    ...
    if not isinstance(candidates, (tuple, list)):
        raise Case1IncompleteError("Case-4 detail response has no orders array")
    matches = tuple(
        item
    ...
    if len(matches) != 1:
        raise Case1IncompleteError("Case-4 detail did not resolve exactly one order")
    return matches[0]
...
class Case4Runner:
    """Exercise Case 4 only through a supplied test transport."""

    def __init__(
        self,
        session_id: str,
        service: WebullSandboxService,
        transport: Case4Transport,
        ...
        self.config = config
        self.clock = clock
        self.case = SmokeCase.SHORT_COVER
        self.order = exact_case4_order(session_id)
        validate_case4_order(session_id, self.order)
    ...
    def _event(
        ...
        if not isinstance(safe, Mapping):
            raise TypeError("redacted Case-4 detail must be a mapping")
        identity = (
            self.session_id,
            self.case.value,
            "COVER_PLACE",
    ...
    def _evidence(
        ...
        if not isinstance(safe, Mapping):
            raise TypeError("redacted Case-4 response must be a mapping")
    ...
    def run(self) -> Case4Result:
        if self.registry.has_call_boundary(self.session_id, self.case):
            raise Case1IncompleteError("Case 4 already crossed a write boundary")
        ...
        self.registry.insert_envelope(
            self.session_id,
            "SMOKE_CASE4_SHORT_POSITION_BEFORE",
            self.clock(),
            position_before,
        )
        self.registry.insert_envelope(
            self.session_id,
            "SMOKE_CASE4_OPEN_ORDERS_BEFORE",
            self.clock(),
            open_before,
        )
        if not 200 <= position_before.status_code < 300:
            raise Case1IncompleteError("Case-4 position preflight failed")
        if WebullSandboxService._positions(position_before, account_id) != {"AAPL": -1}:
            raise Case1IncompleteError("Case 4 requires exactly one short AAPL share")
        if not 200 <= open_before.status_code < 300:
            raise Case1IncompleteError("Case-4 open-order preflight failed")
        if WebullSandboxService._open_orders(open_before, account_id):
            raise Case1IncompleteError("Case 4 requires no working orders")
        evidence = [self._evidence("SHORT_POSITION_BEFORE", position_before, {})]

        preview = self.transport.preview_exact_cover(account_id, self.order)
        self.registry.insert_envelope(
            self.session_id, "SMOKE_CASE4_COVER_PREVIEW", self.clock(), preview, self.order
        )
        if not 200 <= preview.status_code < 300:
            raise Case1IncompleteError("Case-4 cover preview was unsuccessful")
        evidence.append(self._evidence("COVER_PREVIEW", preview))
        ...
        try:
            placed = self.transport.place_exact_cover(account_id, self.order)
            ...
            raise Case1AmbiguousError(
                "Case-4 cover placement was ambiguous; queried once and halted"
            ) from error
        ...
        if not 200 <= placed.status_code < 300:
            raise Case1IncompleteError("Case-4 cover placement was unsuccessful")
        evidence.append(self._evidence("COVER_PLACE", placed))

        detail = self.transport.order_detail(account_id, self.order.client_order_id)
        self.registry.insert_envelope(
            self.session_id, "SMOKE_CASE4_COVER_DETAIL", self.clock(), detail, self.order
        )
        if not 200 <= detail.status_code < 300 or not _detail_matches(detail, self.order):
            raise Case1IncompleteError("Case-4 cover detail identity or fill is invalid")
        evidence.append(self._evidence("COVER_DETAIL", detail))

        position_reduced = self.transport.positions(account_id)
        self.registry.insert_envelope(
            self.session_id,
            "SMOKE_CASE4_POSITION_REDUCED",
            self.clock(),
            position_reduced,
        )
        if not 200 <= position_reduced.status_code < 300:
            raise Case1IncompleteError("Case-4 position reconciliation failed")
        if WebullSandboxService._positions(position_reduced, account_id):
            raise Case1IncompleteError("Case 4 cover must flatten without reversing long")
        evidence.append(self._evidence("POSITION_REDUCED", position_reduced, {}))
        ...
        return Case4Result(capture, self.order.client_order_id)
